...
```
细胞类型嵌入: e_cell = Embedding_cell(cell_type_id) ∈ R^d

组合条件: c_combined = [e_cell; e_pert] ∈ R^{2d}
(细胞类型嵌入与扰动条件拼接)
```
...
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  CrossTypeAttn(Q, K, V) = softmax((QK^T + M) / sqrt(d_k)) * V      │
│                                                                     │
│  其中 M 为细胞类型掩码矩阵:                                         │
│                                                                     │
│  M_ij = 0       (如果相同细胞类型)                                  │
...
```
支持集适应: θ' = θ - α * ∇_θ L_support(θ; D_support)

查询集评估: L_meta = L_query(θ'; D_query)
```
...
│  图神经网络聚合邻居信息:                                            │
│                                                                     │
│  h_c^{(l+1)} = σ(W^{(l)} * AGG({h_{c'}^{(l)} : c' ∈ N(c)}))       │
│                                                                     │
...
- **输入**:
  - 源细胞类型的基因表达数据 x ∈ R^{n_genes}
  - 源细胞类型信息 c_{source}
  - 目标细胞类型信息 c_{target}
  - 扰动条件 c_{pert}
- **输出**:
  - 目标细胞类型中预测的扰动后基因表达 x̂ ∈ R^{n_genes}
