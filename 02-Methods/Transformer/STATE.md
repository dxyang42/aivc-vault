...
STATE使用多头自注意力机制来建模细胞间关系：

1. **Query-Key-Value机制**: 每个细胞嵌入被转换为查询(Query)、键(Key)和值(Value)三个向量
2. **相似度计算**: 通过Query和Key的点积计算细胞间的相似度分数
3. **权重归一化**: 使用Softmax将相似度转换为注意力权重(总和为1)
4. **加权聚合**: 用注意力权重对Value向量加权求和，得到每个细胞的上下文表示
5. **多头并行**: 使用多个注意力头捕捉不同类型的细胞关系，结果拼接后投影
