...
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  输入: x_t (当前状态), t (时间), condition (扰动条件)              │
│                              ↓                                      │
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │              LLaMA-based 细胞编码器                          │   │
│  │  ├── 维度: 1024                                             │   │
...
│  ┌─────────────────────────────────────────────────────────────┐   │
│  │              流速度预测器                                    │   │
│  │  └── 预测速度场 velocity [batch, n_cells, n_genes]          │   │
│  └─────────────────────────────────────────────────────────────┘   │
│                              ↓                                      │
│  输出: 速度场                                                       │
│                                                                     │
...
```
端点损失: L_endpoint = E[||x_1^pred - x_1^true||^2]

其中 x_1^pred 通过 ODE 求解器从 x_0 积分得到:
x_1^pred = ODESolve(v_θ, x_0, t_0=0, t_1=1)
```
...
- **输入**:
  - 单细胞基因表达数据 X ∈ R^{n_cells × n_genes}
  - 扰动条件 c（基因/药物/细胞因子）
- **输出**:
  - 预测的扰动后细胞状态分布 Y ∈ R^{n_cells × n_genes}
