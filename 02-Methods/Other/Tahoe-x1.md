...
│   ├─ x in R^d (d = number of genes, ~20K)                  │
│   └─ Random mask M subset of {1, ..., d}                   │
│                                                             │
│   ↓                                                         │
│                                                             │
│   ┌─────────────────────────────────────────────────────┐   │
...
│   │                                                      │   │
│   │   Masked input:                                      │   │
│   │   x_tilde_i = { x_i      if i not in M              │   │
│   │                { [MASK]  if i in M                  │   │
│   │                                                      │   │
│   │   Gene embeddings:                                   │   │
│   │   e_i = Embed_gene(i) + x_tilde_i · w_i             │   │
│   │                                                      │   │
│   │   Cell embedding:                                    │   │
│   │   e_cell = MLP(metadata)                            │   │
│   │                                                      │   │
...
**Random masking:**
掩码集合M从所有基因子集中均匀随机采样，大小为掩码比例r乘以基因总数d。通常掩码比例在0.15到0.3之间。

### 基因嵌入

基因嵌入由基因ID的查找表嵌入与掩码后表达值的缩放相加得到，捕获基因身份和表达水平信息。

### Transformer编码

输入序列经过L层Transformer编码，每层通过自注意力捕获基因间关系，通过前馈网络进行非线性变换。

### 预测目标

对每个掩码位置，使用MLP从该位置的最终隐状态预测原始表达值。

### 预训练损失

预训练损失是所有掩码位置预测值与真实值之间平方差的和，优化模型重建被掩码基因的能力。

### 下游微调

**Perturbation prediction:**
微调损失是扰动后真实表达与模型预测之间的平方误差，使用预训练编码器和专门的扰动解码器。
