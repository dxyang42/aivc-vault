...
PerturbVAE-Pro 将潜变量分解为三个独立的部分，通过拼接组合成完整的潜变量表示。
...
**证据下界 (ELBO)**: 标准的变分推断目标，包含重构项和 KL 散度正则化
- 重构项: 鼓励解码后的数据与原始数据相似
- KL 项: 鼓励后验分布接近先验分布
- beta 参数: 控制正则化强度

**总相关性惩罚 (TC)**: 鼓励潜变量各维度之间相互独立
- 测量联合分布与边缘分布乘积之间的差异
- 惩罚潜变量之间的相关性
- gamma 参数: 控制解耦强度

**总损失**: ELBO 减去 TC 惩罚（乘以权重系数）

### 可控生成

**扰动效应隔离**: 通过扰动潜变量解码器单独提取扰动效应
- 输入: 扰动潜变量
- 输出: 扰动引起的表达变化

**反事实生成**: 组合细胞状态和新扰动效应生成预测
- 保持细胞状态不变
- 替换扰动效应为新的扰动
- 解码生成扰动后的细胞状态
